...
static uint8 prv_set_tlv(lwm2m_data_t * dataP, conn_s_data_t * connStDataP)
{
    switch (dataP->id) {
...
static uint8 prv_read(uint16 instanceId, int * numDataP, lwm2m_data_t** dataArrayP, lwm2m_object_t * objectP)
{
    uint8 result;
    int i;
...
static uint8 prv_exec(uint16 instanceId, uint16 resourceId,
                        uint8 * buffer, int length, lwm2m_object_t * objectP)
{
    // this is a single instance object
    if (instanceId != 0)
    {
...
void conn_s_updateTxStatistic(lwm2m_context_t * lwm2mH, uint16 txDataByte, bool smsBased)
{
...
void conn_s_updateRxStatistic(lwm2m_context_t * lwm2mH, uint16 rxDataByte, bool smsBased)
{
    lwm2m_object_t *objectP = (lwm2m_object_t *)LWM2M_LIST_FIND(lwm2mH->objectList, LWM2M_CONN_STATS_OBJECT_ID);
    if (objectP == NULL)
        return;
