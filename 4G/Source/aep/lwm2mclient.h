...
void LWM2M_SocketConnect(SOCK_IN_ADDR_T addr, uint16 port);
 ...
 * object_test.c
 */
lwm2m_object_t * get_test_object(uint16_t objid, uint32_t instCount);
...
extern void conn_s_updateTxStatistic(lwm2m_context_t * objectP, uint16_t txDataByte, bool smsBased);
extern void conn_s_updateRxStatistic(lwm2m_context_t * objectP, uint16_t rxDataByte, bool smsBased);
...
int aep_server_bind_device(void);
int aep_server_recv_data(size_t len);
void aep_server_send_data(char *buffer);
