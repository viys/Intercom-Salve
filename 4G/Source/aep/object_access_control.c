...
{   // linked list:
    ...
    uint16                resInstId;  // matches lwm2m_list_t::id, ..serverID
    // resource data:
    uint16                accCtrlValue;
} acc_ctrl_ri_t;
...
{   //linked list:
    ...
    uint16                objInstId;  // matches lwm2m_list_t::id
    // resources
    uint16                objectId;
    uint16                objectInstId;
    uint16                accCtrlOwner;
...
static uint8 prv_set_tlv(lwm2m_data_t* dataP, acc_ctrl_oi_t* accCtrlOiP)
{
    switch (dataP->id) {
...
static uint8 prv_read(uint16 instanceId, int * numDataP,
...
static uint8 prv_write_resources(uint16 instanceId, int numData,
               lwm2m_data_t* tlvArray, lwm2m_object_t* objectP, bool doCreate)
{
    int i;
    uint8 result = COAP_500_INTERNAL_SERVER_ERROR;
    int64_t value;
...
static uint8 prv_create(uint16 objInstId, int numData,
                          lwm2m_data_t * tlvArray, lwm2m_object_t * objectP)
{
    acc_ctrl_oi_t * targetP;
    uint8 result;
...
bool  acc_ctrl_obj_add_inst (lwm2m_object_t* accCtrlObjP, uint16 instId,
                uint16 acObjectId, uint16 acObjInstId, uint16 acOwner)
{
    bool ret = false;
...
bool acc_ctrl_oi_add_ac_val (lwm2m_object_t* accCtrlObjP, uint16 instId,
                             uint16 acResId, uint16 acValue)
{
    bool ret = false;
