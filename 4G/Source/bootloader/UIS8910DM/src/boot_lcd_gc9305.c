...
__UNUSED static void _gc9305SetDirection(lcdDirect_t direct_type)
{

    bootLog("lcd:    _gc9305SetDirection = %d", direct_type);
...
)
{
    ...
    bootLog("lcd:    _gc9305SetDisplayWindow L = %d, top = %d, R = %d, bottom = %d", left, top, right, bottom);
...
static void _gc9305SleepIn(bool is_sleep)
{
    bootLog("lcd:  gc9305  _gc9305SleepIn, is_sleep = %d", is_sleep);
...
static void _gc9305Close(void)
{
    bootLog("lcd:  gc9305   in GC9304_Close");

    _gc9305SleepIn(true);
}
static void _gc9305Invalidate(void)
{
    bootLog("lcd:  gc9305   in _gc9305Invalidate");
...
)
{
    ...
    bootLog("lcd: gc9305 _gc9305InvalidateRect");
    ...
    if ((right < left) || (bottom < top))
    {
        ...
        BOOT_ASSERT(false, "lcd: gc9305 _gc9305InvalidateRect error"); /*assert verified*/
    }
...
static void _gc9305EsdCheck(void)
{
    ...
    bootLog("lcd:  gc9305  gc9305EsdCheck");
}
...
static uint32_t _gc9305ReadId(void)
{
    ...
    HAL_ERR_T r_err;
    uint8_t id[4] = {0};
    bootLog("lcd:  gc9305   _gc9305ReadId \n");
    ...
    _lcdDelayMs(10);
    bootLog("lcd:  gc9305  _gc9305ReadId LCM: 0x%0x, 0x%x, 0x%0x (read return : %d)\n", id[2], id[1], id[3], r_err);

    ret_id = ((id[3] << 16) | (id[2] << 8) | id[1]);

    if (LCD_DRV_ID_GC9305 == ret_id)
    {
        bootLog("lcd:  gc9305 is using !");
    }
