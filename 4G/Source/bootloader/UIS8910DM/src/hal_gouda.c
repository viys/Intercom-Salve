...
uint32_t halGoudaUpdateSerialTiming(uint32_t sysFreq)
{
    ...
    bootLog("lcd:  gouda clk divider set to %d", divider);
...
{
    BOOT_ASSERT(config->b.outputFormat == HAL_GOUDA_LCD_OUTPUT_FORMAT_16_bit_RGB565,
...
HAL_ERR_T halGoudaWriteReg(uint16_t addr, uint16_t data)
{
    ...
    BOOT_ASSERT(g_gouda_ctx.goudaOpened == true, "can not call halGoudaWriteReg before gouda open !");
    status = halGoudaEnterCriticalSection();
...
HAL_ERR_T halGoudaReadData(uint16_t addr, uint8_t *pData, uint32_t len)
{
    ...
    int i;
    HAL_ERR_T errorStatus = HAL_ERR_NO;
    BOOT_ASSERT(len > 0, "Invalid LCD read length:");
    BOOT_ASSERT(g_gouda_ctx.goudaOpened == true, "can not call halGoudaReadData before gouda open !");
    if (g_halGoudaSlcdCfg.b.isSlcd)
    {
        BOOT_ASSERT(len <= GD_MAX_SLCD_READ_LEN, "Invalid SLCD read length: %d (max: %d)");
    }
...
HAL_ERR_T halGoudaReadReg(uint16_t addr, uint16_t *pData)
{
    HAL_ERR_T errorStatus = HAL_ERR_NO;
    uint32_t len;
    BOOT_ASSERT(g_gouda_ctx.goudaOpened == true, "can not call halGoudaReadReg before gouda open !");
    *pData = 0;
    if (g_halGoudaSlcdCfg.b.isSlcd)
    {
        len = 1;
...
HAL_ERR_T halGoudaWriteCmd(uint16_t addr)
{
    ...
    BOOT_ASSERT(g_gouda_ctx.goudaOpened == true, "can not call halGoudaWriteCmd before gouda open !");
    status = halGoudaEnterCriticalSection();
...
HAL_ERR_T halGoudaWriteData(uint16_t data)
{
    uint32_t status;
    HAL_ERR_T errorStatus = HAL_ERR_NO;
    BOOT_ASSERT(g_gouda_ctx.goudaOpened == true, "can not call halGoudaWriteData before gouda open !");
    status = halGoudaEnterCriticalSection();
...
HAL_ERR_T halGoudaBlitRoi(const HAL_GOUDA_WINDOW_T *pRoi, const uint32_t nbCmd, const HAL_GOUDA_LCD_CMD_T *pCmds, HAL_GOUDA_IRQ_HANDLER_T handler)
{
    ...
    BOOT_ASSERT((g_gouda_ctx.goudaOpened == true), "can not call halGoudaBlitRoi before gouda open !");
    BOOT_ASSERT((pRoi != NULL), "halGoudaBlitRoi pRoi must not be NULL!");
    BOOT_ASSERT(((pRoi->tlPX < pRoi->brPX) && (pRoi->tlPY < pRoi->brPY)),
...
HAL_ERR_T halGoudaBlitRoi2Ram(uint32_t *pBuffer, const uint16_t width, const HAL_GOUDA_WINDOW_T *pRoi, HAL_GOUDA_IRQ_HANDLER_T handler)
{
    ...
    BOOT_ASSERT(g_gouda_ctx.goudaOpened, "can not call halGoudaBlitRoi2Ram before gouda open !");
    BOOT_ASSERT(pRoi != NULL, "halGoudaBlitRoi2Ram pRoi must not be NULL!");
    BOOT_ASSERT(((pRoi->tlPX < pRoi->brPX) && (pRoi->tlPY < pRoi->brPY)),
    ...
    if (width != 0)
    {
        ...
        BOOT_ASSERT(width >= roiWidth, "halGoudaBlitRoi2Ram width (%d) must be greater or equal to width of roi (%d)");
        // offset is in byte
...
HAL_ERR_T halGoudaOvlLayerOpen(HAL_GOUDA_OVL_LAYER_ID_T id, HAL_GOUDA_OVL_LAYER_DEF_T *def)
{
    ...
    BOOT_ASSERT(id < HAL_GOUDA_OVL_LAYER_ID_QTY, "halGoudaOvlLayerOpen invalid ID:%d");
    ...
    BOOT_ASSERT(!((def->pos.tlPX > def->pos.brPX) || (def->pos.tlPY > def->pos.brPY)),
...
void halGoudaOvlLayerClose(HAL_GOUDA_OVL_LAYER_ID_T id)
{
    ...
    BOOT_ASSERT(id < HAL_GOUDA_OVL_LAYER_ID_QTY, "halGoudaOvlLayerClose invalid ID:%d");
...
HAL_ERR_T halGoudaVidLayerOpen(HAL_GOUDA_VID_LAYER_DEF_T *def)
{
    ...
    BOOT_ASSERT(((xpitch < 0x800) && (ypitch < 0x800)), "hal_GoudaVidLayerOpen image zoom ratio out of range (0x%0x,0x%0x)");
...
void halGoudaIrqHandler(void *interruptId)
{
    uint32_t tmpRead;
...
{
    bootLog("lcd:  halGoudaInit clk=%d", clk);
