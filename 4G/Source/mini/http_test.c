...
// #define HTTP_URL "http://116.62.226.17:9988/fota.img"
    ...
    while(1)
    {
        ...
        if (httpMsgHandle != NULL)
        {
            ...
            else if (msg.messageID == 122)
            {
                ...
                ret = KING_FsInit();
                LogPrintf("write init file , ret %d\r\n", ret);
                ret = KING_FsFileCreate("fota.img", FS_MODE_CREATE_ALWAYS | FS_MODE_WRITE | FS_MODE_READ, 0, 0, &FileHandle);
                LogPrintf("write create file , ret %d\r\n", ret);
                while (!readEnd)
                {
                    memset(pData, 0x00, 1024 + 1);
                    ret = http_readContentDataAndLen(60, 1024, pData,&bytesRead, &readEnd);
                    if (ret != 0)
                    {
                        free(pData);
                        pData = NULL;
                    ...
                    // if(0 > bytesRead){
                    //     KING_FotaFlashWrite(0, phttpData, (uint32)bytesRead, &bytesWr);
                    //     size+=bytesRead;
                    // }
                    KING_FsFileWrite(FileHandle, pData, bytesRead , &size);
                    LogPrintf("write file size %d , ret %d\r\n", size, ret);
                    LogPrintf("http_get_callback readEnd : %d", readEnd);
                    LogPrintf("http_get_callback pData :%d, %s",bytesRead,pData);
                    // KING_Sleep(70);
                }
