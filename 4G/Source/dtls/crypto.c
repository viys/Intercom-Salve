...
{
    dtls_handshake_parameters_t *p = NULL;
    KING_MemAlloc((void **)&p, sizeof(dtls_handshake_parameters_t));
    return p;
}

static inline void dtls_handshake_dealloc( dtls_handshake_parameters_t *handshake )
{
    KING_MemFree( handshake );
}

static inline dtls_security_parameters_t* dtls_security_malloc()
{
    dtls_security_parameters_t *p = NULL;
    KING_MemAlloc((void **)&p, sizeof(dtls_security_parameters_t));
    return p;
}

static inline void dtls_security_dealloc( dtls_security_parameters_t *security )
{
    KING_MemFree( security );
}
...
dtls_handshake_parameters_t *dtls_handshake_new( void )
{
    ...
    memset( handshake,0, sizeof(dtls_handshake_parameters_t));//dtls_handshake_parameters_t
               ...
               unsigned char       *buf )
{
    ...
    dtls_hmac_update( hmac_ctx, record + 3, sizeof(dtls_uint16)+sizeof(dtls_uint48) );
    dtls_hmac_update( hmac_ctx, record, sizeof(dtls_uint8)+sizeof(dtls_uint16) );
    dtls_hmac_update( hmac_ctx, L, sizeof(dtls_uint16) );
...
dtls_psk_pre_master_secret(unsigned char *key, size_t keylen,
  ...
  if (result_len < (2 * (sizeof(dtls_uint16) + keylen))) {
    return -1;
  }
  ...
  memcpy(p, result, sizeof(dtls_uint16));
  p += sizeof(dtls_uint16);
  
  memcpy(p, key, keylen);

  return 2 * (sizeof(dtls_uint16) + keylen);
}
#endif /* DTLS_PSK */
...
dtls_ecdsa_create_sig_hash(const unsigned char *priv_key, size_t key_size,
  ...
  do {
    dtls_prng((unsigned char *)randNum, key_size);
    ret = ecc_ecdsa_sign(priv, hash, randNum, point_r, point_s);
  } while (ret);
}
