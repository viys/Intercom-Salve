...
static void at_CmuxParseCtrl(AT_CMUX_ENGINE_T *th, AT_CMUX_FRAME_T *frame)
{
    ...
    case CTRL_MSG_CLD:
    {
        ...
        at_TaskCallback((AT_CALLBACK_FUNC_T)at_DispatchEndCmuxMode, th->dispatch);
        break;
    }
    case CTRL_MSG_TEST:
    {
...
static int at_CmuxBasicPushData(AT_CMUX_ENGINE_T *th, const uint8_t *data, unsigned length)
{
    ...
    while (length > 0)
    {
        ...
        {
            ATLOGW("CMUX basic overflow len=%d", th->buffLen);
            th->buffLen = 0;
        }

        if (th->buffLen == 0 && c != BASIC_FLAG)
        {
            ATLOGV("CMUX basic drop byte before sync");
            continue;
        }
        if (th->buffLen == 1 && c == BASIC_FLAG)
        {
            ATLOGD("CMUX basic drop dup sync byte");
            continue;
        }
