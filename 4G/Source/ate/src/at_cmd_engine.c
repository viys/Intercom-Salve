...
{
    AT_CMD_ENGINE_T *th = (AT_CMD_ENGINE_T *)malloc(sizeof(AT_CMD_ENGINE_T));

    th->mode = CMD_PARSE_MODE_LINE;
    th->dispatch = dispatch;
    th->channelId = at_DispatchGetChannel(dispatch);
    th->errSync = false;
    th->buff = (uint8_t *)malloc(CMD_LINE_BUFF_SIZE);
    th->buffLen = 0;
...
static void at_CmdPromptFinished(AT_CMD_ENGINE_T *th)
{
    ...
    at_ModuleRunCommandExtra(th, (const char *)th->buff, th->buffLen);
}
...
{
    AT_CMD_ENGINE_T *th = at_DispatchGetCmdEngine((AT_DISPATCH_T*)param);
...
static void at_CmdLFTimeoutFun(void *param)
{
    void *ctx;
    TIMER_HANDLE handle = (TIMER_HANDLE)param;
    
    ctx = at_GetTimerCtx(handle);
    at_TaskCallback(at_CmdLFTimeout, ctx);
}
...
static int at_CmdPushInLineMode(AT_CMD_ENGINE_T *th, const uint8_t *data, unsigned length)
{
    ...
    if (length > 0)
        at_StopCallbackTimer(at_CmdLFTimeoutFun, th->dispatch);
        ...
        default:
            if (th->buffLen > 0 && th->buff[th->buffLen - 1] == CHAR_CR)
            {
                ...
                if (gATCurrenteCommandEchoMode)
                {
                    if(c == CHAR_LF)
                    {
                        at_CmdWrite(th, data - (total - length), total - length - 1);
                    }
                    else
                    {
                        at_CmdWrite(th, data - (total - length), total - length);
            ...
            if (c == CHAR_CR)
            {
                if (length == 0)
                {
                    at_StartCallbackTimer(LF_TIMER_PERIOD, at_CmdLFTimeoutFun, th->dispatch);
                    break;
                }
                else
                {
...
void at_CmdCommandFinished(AT_CMD_ENGINE_T *th)
{
    if (th->module.cmds == NULL)
    {
        ...
        at_DispatchScheduleDrain(th->dispatch);
    }
}
...
struct AT_DISPATCH_T *at_CmdGetDispatch(AT_CMD_ENGINE_T *th) { return th->dispatch; }
