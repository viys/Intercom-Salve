...
VOID AT_Util_TrimAll_Param(UINT8 *szParam)
{
    ...
    if (szParam == NULL || (len = strlen((const char*)szParam)) <= 0)
        return;
...
VOID AT_Util_TrimRspStringSuffixCrLf(UINT8 *buff, UINT16 *plen)
{
    ...
    while (*plen > 0 && (buff[*plen - 1] == AT_CMD_CR || buff[*plen - 1] == AT_CMD_LF))
    {
        ...
        KING_SysLog("Unwanted CR and/or LF suffix character is found in the RSP string!!! rsp=%s", s);
        buff[*plen--] = 0;
    }
}
...
char** getargv(char* param ,int* count) {
	int j=1;
    char** argv = NULL;
    KING_MemAlloc((void **)&argv, 1000);
	argv[0]="abc";
	KING_SysLog( "enterring getargv");
	
	   char* token = strtok( param, " ");
	    KING_SysLog( "strtok");
	   while( token != NULL )
	   {
		   KING_SysLog( "TOKEN= %s ", token );
		   argv[j]=token;
		   j++;    
		   token = strtok( NULL, " ");
		   KING_SysLog( "j= %d ", j );
...
INT32 AT_Util_GetParaWithRule(UINT8 *pRes, CONST UINT8 iIndex, CONST AT_UTIL_PARA_TYPE eType, PVOID pPara, UINT16 *pLen)
{
    ...
    else
    {
        ...
        if (AT_UTIL_PARA_TYPE_UINT8 == eType)
        {
            ...
            {
                iNmbValue8 = (UINT8)AT_StrAToI((const char*)szNum);
        ...
        else if (AT_UTIL_PARA_TYPE_UINT16 == eType)
        {
            ...
            {
                iNmbValue16 = (UINT16)AT_StrAToI((const char*)szNum);
        ...
        else if(AT_UTIL_PARA_TYPE_UINT32 == eType)
        {
            ...
            {
                iNumValue32 = AT_StrAToI((const char*)szNum);
        ...
        {
            KING_SysLog("exe     AT_Util_CheckValidateString  failed\n");
            return ERR_AT_UTIL_PARA_ERROR;
        }
        return ERR_AT_UTIL_FAILED;
    }
...
BOOL AT_UtilStringToOctet(UINT8 *src_text_string, UINT16 input_len, UINT8 *dst_real_octet)
{
    ...
    length = strlen((const char* )src_text_string);
...
BOOL AT_Util_BitStrToByte(UINT8 *byteDest, CONST UINT8 *strSource)
{
    ...
    if (iLen > 8)
    {
        KING_SysLog("AT_Util_BitStrToByte: Strlen is larger than 8, [%s] length is %d", 
            strSource, iLen);
        return FALSE;
    }
    ...
    for (i = 0; i < iLen; i++)
    {
        ...
        {
            KING_SysLog("AT_Util_BitStrToByte: the %dth[%c] is illegal char of [%s]",
...
BOOL AT_Util_ByteToBitStr(UINT8 *bitStrDest, CONST UINT8 byteSource, UINT8 bitLen)
{
    ...
    if (NULL == bitStrDest)
    {
        KING_SysLog("AT_Util_ByteToBitStr: bitStrDest NULL");
        return FALSE;
    }

    if (bitLen > 8 || bitLen == 0)
    {
        KING_SysLog("AT_Util_ByteToBitStr: bitLen=%d is illegal", bitLen);
        return FALSE;
    }
