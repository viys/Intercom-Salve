...
/// InfoTextBegin/OutputText/InfoTextEnd:
/// * InfoText will add leading and trailing \r\n. So, when InfoText is output
///   by multiple API calls will insert \r\n. In some cases, it is not wanted.
///
/// NOTE:
...
//In cmux mode, output from cmux1 by default
...
void at_CmdRespInfoTextBegin(AT_CMD_ENGINE_T *engine, const uint8_t *text);
void at_CmdRespInfoNTextBegin(AT_CMD_ENGINE_T *engine, const uint8_t *text, unsigned length);
void at_CmdRespInfoTextEnd(AT_CMD_ENGINE_T *engine, const uint8_t *text);
