...
static int skel_valuedecode(lwm2m_data_t * dataP, uint8 ** bufferP)
{
    ...
    case LWM2M_TYPE_BOOLEAN:
    {
        ...
        char *sv = v ? "1": "0";
        *bufferP = (uint8 *)lwm2m_malloc(KING_strlen(sv)+1);
        if (NULL == *bufferP) return -1;
        KING_strcpy(*bufferP,sv);
        return KING_strlen(sv);
    }
                        ...
                        lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 1)
    {
        ...
        if (targetP->dataArray.resId == dataP->id)
        {
            ...
            {
                case LWM2M_TYPE_STRING:
                    lwm2m_data_encode_string(data, dataP);
                    break;
                case LWM2M_TYPE_OPAQUE:
                    lwm2m_data_encode_opaque(data, len, dataP);
                    break;
                case LWM2M_TYPE_INTEGER:
                    lwm2m_data_encode_int(KING_atoi((const char *)data), dataP);
                    break;
                case LWM2M_TYPE_FLOAT:
                    lwm2m_data_encode_float(targetP->dataArray.floatValue, dataP);
                    break;
                case LWM2M_TYPE_BOOLEAN:
                    lwm2m_data_encode_bool(KING_atoi((const char *)data), dataP);
                    break;
                default:
                    return COAP_406_NOT_ACCEPTABLE;
            }
                         ...
                         lwm2m_object_t * objectP)
{
    ...
    if (numData == 1)
    {
        ...
        if (dataArray[i].id == targetP->dataArray.resId)
        {
            ...
            LogPrintf("prv_write dataArray[%d].id=%d,type=%d",i,dataArray[i].id, dataArray[i].type);
            switch (targetP->dataArray.type)
            {
                case LWM2M_TYPE_OPAQUE:
                case LWM2M_TYPE_STRING:
                ...
                case LWM2M_TYPE_BOOLEAN:
                    {
                        ...
                        //LogPrintf("prv_write len=%d, data=%s",len, data);
                    }
