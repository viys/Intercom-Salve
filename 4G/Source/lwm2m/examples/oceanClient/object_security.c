...
typedef struct _security_instance_
{
    ...
    uint16                     serverPublicKeyLen;
    char *                       secretKey;
    uint16                     secretKeyLen;
    uint8                      smsSecurityMode;
    char *                       smsParams; // SMS binding key parameters
    uint16                     smsParamsLen;
    char *                       smsSecret; // SMS binding secret key
    uint16                     smsSecretLen;
    uint16                     shortID;
    uint32                     clientHoldOffTime;
    uint32                     bootstrapServerAccountTimeout;
} security_instance_t;

static uint8 prv_get_value(lwm2m_data_t * dataP,
                             security_instance_t * targetP)
{
    ...
    case LWM2M_SECURITY_PUBLIC_KEY_ID:
        lwm2m_data_encode_opaque((uint8*)targetP->publicIdentity, targetP->publicIdLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SERVER_PUBLIC_KEY_ID:
        lwm2m_data_encode_opaque((uint8*)targetP->serverPublicKey, targetP->serverPublicKeyLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SECRET_KEY_ID:
        lwm2m_data_encode_opaque((uint8*)targetP->secretKey, targetP->secretKeyLen, dataP);
        return COAP_205_CONTENT;
    ...
    case LWM2M_SECURITY_SMS_KEY_PARAM_ID:
        lwm2m_data_encode_opaque((uint8*)targetP->smsParams, targetP->smsParamsLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SMS_SECRET_KEY_ID:
        lwm2m_data_encode_opaque((uint8*)targetP->smsSecret, targetP->smsSecretLen, dataP);
        return COAP_205_CONTENT;
...
static uint8 prv_security_read(uint16 instanceId,
                                 int * numDataP,
                                 ...
                                 lwm2m_object_t * objectP)
{
    ...
    if (*numDataP == 0)
    {
        uint16 resList[] = {LWM2M_SECURITY_URI_ID,
        ...
        int nbRes = sizeof(resList)/sizeof(uint16);
...
static uint8 prv_security_write(uint16 instanceId,
                                  int numData,
                                  ...
                                  lwm2m_object_t * objectP)
{
    ...
    do {
        ...
        case LWM2M_SECURITY_URI_ID:
            ...
            if (targetP->uri != NULL)
            {
                KING_strncpy(targetP->uri, (char*)dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
        ...
        case LWM2M_SECURITY_PUBLIC_KEY_ID:
            ...
            {
                KING_memcpy(targetP->publicIdentity, (char*)dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
        ...
        case LWM2M_SECURITY_SERVER_PUBLIC_KEY_ID:
            ...
            {
                KING_memcpy(targetP->serverPublicKey, (char*)dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
        ...
        case LWM2M_SECURITY_SECRET_KEY_ID:
            ...
            {
                KING_memcpy(targetP->secretKey, (char*)dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
...
static uint8 prv_security_delete(uint16 id,
                                   lwm2m_object_t * objectP)
{
    security_instance_t * targetP;
...
static uint8 prv_security_create(uint16 instanceId,
                                   int numData,
                                   lwm2m_data_t * dataArray,
                                   lwm2m_object_t * objectP)
{
...
{
    KING_memcpy(objectDest, objectSrc, sizeof(lwm2m_object_t));
    ...
    while (instanceSrc != NULL)
    {
        ...
        KING_memcpy(instanceDest, instanceSrc, sizeof(security_instance_t));
        instanceDest->uri = (char*)lwm2m_malloc(KING_strlen(instanceSrc->uri) + 1);
        KING_strcpy(instanceDest->uri, instanceSrc->uri);
                                     ...
                                     bool isBootstrap)
{
    ...
    if (NULL != securityObj)
    {
        ...
        targetP->uri = (char*)lwm2m_malloc(KING_strlen(serverUri)+1); 
        KING_strcpy(targetP->uri, serverUri);
        ...
        if (bsPskId != NULL || psk != NULL)
        {
            ...
            if (bsPskId)
            {
                ...
                targetP->publicIdLen = KING_strlen(bsPskId);
            }
            if (pskLen > 0)
            {
                ...
                KING_memcpy(targetP->secretKey, psk, pskLen);
