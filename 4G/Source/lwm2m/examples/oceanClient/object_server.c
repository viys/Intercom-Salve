...
typedef struct _server_instance_
{
    ...
    uint16    shortServerId;
    uint32    lifetime;
    uint32    defaultMinPeriod;
    uint32    defaultMaxPeriod;
...
static uint8 prv_server_read(uint16 instanceId,
                               int * numDataP,
...
static uint8 prv_server_discover(uint16 instanceId,
                                   int * numDataP,
                                   lwm2m_data_t ** dataArrayP,
                                   lwm2m_object_t * objectP)
{
...
static uint8 prv_set_int_value(lwm2m_data_t * dataArray, uint32 * data) {
    uint8 result;
    int64_t value;
...
static uint8 prv_server_write(uint16 instanceId,
                                int numData,
                                ...
                                lwm2m_object_t * objectP)
{
    ...
    do
    {
        ...
        case LWM2M_SERVER_SHORT_ID_ID:
            {
                uint32 value = targetP->shortServerId;
        ...
        case LWM2M_SERVER_LIFETIME_ID:
            result = prv_set_int_value(dataArray + i, (uint32 *)&(targetP->lifetime));
            break;
        ...
        case LWM2M_SERVER_BINDING_ID:
            ...
            {
                KING_strncpy(targetP->binding, (char*)dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
                result = COAP_204_CHANGED;
            }
            else
            {
...
static uint8 prv_server_execute(uint16 instanceId,
                                  uint16 resourceId,
                                  uint8 * buffer,
                                  int length,
                                  lwm2m_object_t * objectP)
...
static uint8 prv_server_delete(uint16 id,
                                 lwm2m_object_t * objectP)
{
...
static uint8 prv_server_create(uint16 instanceId,
                                 int numData,
                                 lwm2m_data_t * dataArray,
                                 lwm2m_object_t * objectP)
{
...
void copy_server_object(lwm2m_object_t * objectDest, lwm2m_object_t * objectSrc)
{
    ...
    while (instanceSrc != NULL)
    {
        ...
        KING_memcpy(instanceDest, instanceSrc, sizeof(server_instance_t));
        // not sure it's necessary:
        KING_strcpy(instanceDest->binding, instanceSrc->binding);
                                   ...
                                   bool storing)
{
    ...
    if (NULL != serverObj)
    {
        ...
        KING_memcpy (serverInstance->binding, binding, KING_strlen(binding)+1);
#ifndef LWM2M_VERSION_1_0
        serverInstance->registrationPriorityOrder = -1;
        serverInstance->initialRegistrationDelayTimer = -1;
        serverInstance->registrationFailureBlock = -1;
