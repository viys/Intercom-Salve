...
typedef struct
{
    ...
    float    altitude;
    float    radius;
    uint8  velocity   [VELOCITY_OCTETS];        //3GPP notation 1st step: HORIZONTAL_VELOCITY_WITH_UNCERTAINTY
...
*/
static uint8 prv_res2tlv(lwm2m_data_t* dataP,
                           location_data_t* locDataP)
{
    //-------------------------------------------------------------------- JH --
    uint8 ret = COAP_205_CONTENT;  
...
/**
  ...
  */
static uint8 prv_location_read(uint16 objInstId,
                                 int*  numDataP,
...
{   
    ...
    if (*numDataP == 0)     // full object, readable resources!
    {
        ...
        *numDataP  = sizeof(readResIds)/sizeof(uint16);
