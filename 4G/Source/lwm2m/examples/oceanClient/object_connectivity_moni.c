...
lwm2m_object_t * get_object_conn_m(void)
{
    ...
    if (NULL != connObj)
    {
        ...
        if (NULL != connObj->userData)
        {
            ...
            KING_strcpy(myData->ipAddresses[0],       VALUE_IP_ADDRESS_1);
            KING_strcpy(myData->ipAddresses[1],       VALUE_IP_ADDRESS_2);
            KING_strcpy(myData->routerIpAddresses[0], VALUE_ROUTER_IP_ADDRESS_1);
            KING_strcpy(myData->routerIpAddresses[1], VALUE_ROUTER_IP_ADDRESS_2);
        }
        else
        {
            lwm2m_free(connObj);
...
uint8 connectivity_moni_change(lwm2m_data_t * dataArray,
                                 lwm2m_object_t * objectP)
{
    int64_t value;
    uint8 result;
    ...
    case RES_M_IP_ADDRESSES:
        ...
        else
        {
            ...
            KING_memcpy(data->ipAddresses[0], dataArray->value.asBuffer.buffer, dataArray->value.asBuffer.length);
    ...
    case RES_O_ROUTER_IP_ADDRESS:
        ...
        else
        {
            ...
            KING_memcpy(data->routerIpAddresses[0], dataArray->value.asBuffer.buffer, dataArray->value.asBuffer.length);
