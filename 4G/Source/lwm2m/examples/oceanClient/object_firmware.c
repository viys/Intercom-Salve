...
#define LWM2M_FIRMWARE_PROTOCOL_NULL    ((uint8)-1)
...
typedef struct
{
    ...
    uint8 protocol_support[LWM2M_FIRMWARE_PROTOCOL_NUM];
    uint8 delivery_method;
} firmware_data_t;

static uint8 prv_firmware_read(uint16 instanceId,
                                 int * numDataP,
...
static uint8 prv_firmware_write(uint16 instanceId,
                                  int numData,
...
static uint8 prv_firmware_execute(uint16 instanceId,
                                    uint16 resourceId,
                                    uint8 * buffer,
                                    int length,
                                    lwm2m_object_t * objectP)
...
lwm2m_object_t * get_object_firmware(void)
{
    ...
    if (NULL != firmwareObj)
    {
        ...
        if (NULL != firmwareObj->userData)
        {
            ...
            data->state = 1;
            data->result = 0;
            KING_strcpy(data->pkg_name, "lwm2mclient");
            KING_strcpy(data->pkg_version, "1.0");
