...
//    GPIO_CFG_S cfg;
    ...
    if (KING_FsFileCreate("fota.bin", FS_MODE_OPEN_EXISTING, 0, 0, &fd) < 0)
    {
        AppLog("open fota.bin fail\r\n");
        return;
    }

    if (KING_FsFileSizeGet(fd, &flsz) < 0)
    {
        AppLog("get fota.bin size fail\r\n");
        KING_FsCloseHandle(fd);
        return;
    }
    else
    {
        AppLog("fota.bin size is %d\r\n", flsz);
        if (flsz == 0)
        {
            return;
        }
