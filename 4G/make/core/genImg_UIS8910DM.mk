...
PRESET_GEN:
	@${ECHO} "Generate preset file"
	$(PYTHON) ${PRESETGEN} --source-top ${TOPDIR} --binary-top ${TOPDIR}Build --prepackfile ${PRESETCFG} ${TOPDIR}prebuilts/${PLATFORM_NAME}/preset.cpio

############################ hex files #######################################
# Generate srec and disassembly files, same logic as bin
# Always build APPBIN
APPBIN: force PRESET_GEN
	${MAKE} ${BIN_FINAL}
	@${ECHO} ""
	...
	cp -f ${BAS_FINAL}.img ${TOPDIR}prebuilts/${PLATFORM_NAME}/UIS8910DM_release.img
	$(PYTHON) $(PACGEN) kingapp-gen --cfg ${PACK_JSON_PATH} ${BAS_FINAL}.pac
	del .\prebuilts\${PLATFORM_NAME}\UIS8910DM_release.img

	cp -f ${TOPDIR}prebuilts/delete.img ${TOPDIR}prebuilts/${PLATFORM_NAME}/UIS8910DM_release.img
	$(PYTHON) $(PACGEN) kingapp-gen --cfg ${PACK_JSON_PATH} ${BINARY_PATH}/delete_app.pac
	del .\prebuilts\${PLATFORM_NAME}\UIS8910DM_release.img
	del .\prebuilts\${PLATFORM_NAME}\preset.cpio
